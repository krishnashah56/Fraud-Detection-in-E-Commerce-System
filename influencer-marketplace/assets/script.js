...
jQuery(function($) {
    ...
    // Auto-fill filters from URL and trigger search if on search page
    if ($('#im-search-results').length > 0) {
        const urlParams = new URLSearchParams(window.location.search);
        let hasParams = false;
        
        if (urlParams.has('category')) {
            $('#filter_category').val(urlParams.get('category'));
            hasParams = true;
        }
        if (urlParams.has('location')) {
            $('#filter_location').val(urlParams.get('location'));
            hasParams = true;
        }

        if (hasParams) {
            window.imSearchInfluencers();
        }
    }
